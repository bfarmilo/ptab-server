const { setStatus, fixDate, makeFWDStatus, getPatentOwners, mapPatentClaim, importPTAB } = require('./loadMongo');
...
connect()
  .then(dbObject => {
    db = dbObject;
    collection = dbObject.collection('newTrials');
    return;
  })
  .then(() => {
    // then main function goes here
    return importPTAB(db, collection);
